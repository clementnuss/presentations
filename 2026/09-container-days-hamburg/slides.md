...
<!--
* Now that we have 3 CP nodes, let's use some cool Feature..
* Floating IP for failover.
* Very useful if you don't have a external LB
* We don't wanna put this patch in all/....
* Another example where machine config might differ...
* Once again topf apply...
-->
...
<!--
* You might ask, that's simple, why not just talosctl
* Yes it's possible, and also how we started out
* Example from previous slide

* secrets bundle generation and bootstrap: you only wanna do that once, TOPF is smart about that, that auto-bootstrap flag also figures out if it needs to send that command
* you'd have to list all patches individually. probably would need to script something to populate dynamically, or you WILL forget
* You have to manually loop through all nodes, know which one are worker and CP, and which ip
* IMPORTANT: second time, --insecure needs to go. 
* For the sake of argument, one of those two worker is slightly different, and has a different disk layout. So that disk patch, needs to be different. No easy way! Let's look how we could solve this with TOPF
-->
...
This is good for single node differences, but what if ALL your nodes need different config. You don't wanna have 100 folders here. For this introduce the probably most powerful feature... TEMPLATING
-->
...
<!--
You know Helm, you love it, you hate it, but it's powerful.

If your patch ends with .tpl, you can template it!

Any data from topf.yaml is available. Not only static ones but from list of nodes.

Classic Example: Hostname, something that is different per node

Default Talos: autogenerated

Knowing that, let's refactor our setting a bit...
-->
...
<!--
... by allowing users to set arbitrary key value data in topf.yaml

define the disk for each node...

and use it in a template

powerful pattern

Segway: so let's say you're convinced wanna try that. Put your cluster config in some git repo and then you realize...
-->
...
<!--
This is the one that's hard to understand if you haven't used the Talos Image Factory. Normally you go to factory.talos.dev, click through a wizard to pick kernel args
